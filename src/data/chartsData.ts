...
export const chartsData: ChartMeta[] = [
  ...
  // ===== 20. 弦图 =====
  {
    ...
    dataRequirements: '需要**有方向的**源列、目标列、数值列。同一对节点 (A,B) 的 A→B 与 B→A 是两条独立记录，数值可不同——这正是弦图"两端带宽不等"的核心数据特征',
    exampleData: {
      // "某学年四所学校之间的转学人数"，有向流量矩阵的行列表式：
      //   每行 [转出, 转入, 人数]，每对学校的进出方向独立
      //   例如 A→B=10 但 B→A=5，A 流失 10、接收 5，弦两端粗细不等
      columns: ['转出', '转入', '人数'],
      rows: [
        // A 学校流出
        ['A学校', 'B学校', 10], ['A学校', 'C学校', 15], ['A学校', 'D学校', 10],
        // B 学校流出
        ['B学校', 'A学校', 5],  ['B学校', 'C学校', 15], ['B学校', 'D学校', 5],
        // C 学校流出
        ['C学校', 'A学校', 5],  ['C学校', 'B学校', 10], ['C学校', 'D学校', 15],
        // D 学校流出
        ['D学校', 'A学校', 10], ['D学校', 'B学校', 10], ['D学校', 'C学校', 10],
      ],
    },
    generatorSupported: true,
    defaultMapping: { sourceField: '转出', targetField: '转入', valueField: '人数' },
  },
  ...
  // ===== 21. 六边形分箱图 =====
  {
    ...
    dataRequirements: '需要两个数值列（X 和 Y），**数据量越大（几十~几万点）越能体现密度分布的意义**',
    exampleData: {
      // ~70 个模拟"商圈顾客位置"散点，围绕 3 个购物热区聚集
      // 数据足够多才能展现"密度分箱+颜色热度"的效果，对应定义里
      // "当散点过多重叠时用六边形统计频数"
      columns: ['X坐标', 'Y坐标'],
      rows: [
        // 热区 A：中心 (3, 7)，约 25 点
        [2.5, 7.0], [2.8, 7.3], [3.2, 7.2], [3.5, 6.8], [2.9, 6.5],
        [3.3, 7.5], [2.6, 7.8], [3.1, 7.0], [2.7, 6.9], [3.4, 6.6],
        [2.2, 7.1], [3.6, 7.4], [3.0, 7.9], [2.4, 6.7], [3.3, 6.3],
        [2.8, 7.6], [3.2, 7.1], [2.5, 6.5], [3.1, 7.7], [3.5, 7.2],
        [2.7, 7.3], [2.9, 6.8], [3.4, 7.1], [3.0, 6.4], [2.6, 7.5],
        // 热区 B：中心 (5, 5)，约 25 点（最密集）
        [4.5, 5.0], [5.2, 4.8], [4.8, 5.3], [5.1, 4.5], [4.7, 5.5],
        [5.4, 5.1], [4.6, 4.9], [5.3, 5.4], [4.9, 5.2], [5.0, 4.7],
        [4.4, 5.3], [5.5, 4.6], [5.2, 5.6], [4.8, 4.4], [5.0, 5.1],
        [4.6, 5.2], [5.3, 4.9], [4.9, 5.5], [5.1, 5.0], [4.7, 4.6],
        [5.4, 5.3], [4.5, 4.8], [5.2, 5.1], [4.8, 5.0], [5.0, 4.9],
        // 热区 C：中心 (8, 2)，约 12 点
        [7.5, 2.0], [8.2, 1.8], [7.8, 2.3], [8.1, 1.6], [7.9, 2.2],
        [8.3, 2.1], [7.7, 1.9], [8.0, 2.4], [7.6, 2.1], [8.2, 2.3],
        [7.9, 1.7], [8.1, 2.0],
        // 散点：10 个分散位置，作为"低频区"对比
        [1.0, 3.0], [9.5, 8.0], [0.5, 9.0], [6.5, 1.5], [1.5, 5.5],
        [9.0, 4.0], [2.0, 1.5], [7.0, 9.0], [0.8, 6.5], [6.0, 7.5],
      ],
    },
    generatorSupported: true,
    defaultMapping: { xField: 'X坐标', yField: 'Y坐标' },
  },
]
