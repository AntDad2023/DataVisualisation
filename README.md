...
测试覆盖 `src/utils/` 下的纯函数：数据解析（CSV / 粘贴）、列类型识别（80% 阈值）、全部 11 种图表的 option 生成器（含直方图分箱、箱线图五数概括、雷达图 max 计算等易出错的数值逻辑）。当前 **13 个测试文件 / 75 个用例**。
...
- [x] GitHub Pages 部署上线
- [x] 核心 utils 单元测试覆盖（Vitest，13 文件 / 75 用例）
